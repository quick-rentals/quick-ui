...
prevent_conflict_markers() {
  ...
  occurrences=($(git --no-pager diff --staged --ignore-blank-lines --name-only -G"$CONFLICT_MARKERS"))

  for file in "${occurrences[@]}"; do
    occurrences_count=$(grep -Ev '^[[:space:]]*#' "$file" | grep -E "$CONFLICT_MARKERS" | wc -l)
    if [ "$occurrences_count" -gt 0 ]; then
